...
// Types for the hospital data we receive
type HospitalPin = {
  id: number;
  name: string;
  lat: number;
...
function MapInner({ hospitals, userLat, userLng, onSelectHospital }: Props) {
  ...
  useEffect(() => {
    ...
    if (userLat && userLng) {
      const userIcon = L.divIcon({
        html: `<div style="width:16px;height:16px;background:#1A73B5;border:3px solid white;border-radius:50%;box-shadow:0 2px 6px rgba(0,0,0,0.3);"></div>`,
        className: "",
        iconSize: [16, 16],
        iconAnchor: [8, 8],
      });
    ...
    // Hospital markers
    hospitals.forEach((h) => {
      const color = h.isPremium ? "#16A085" : "#64748B";
      ...
      marker.bindPopup(`
        <div style="font-family:system-ui;min-width:160px;">
          ...
          <span style="font-size:11px;color:#64748B;">${levelLabel[h.level] || h.level}</span>
          ...
          <br/><button onclick="window.__hpSelectHospital && window.__hpSelectHospital(${h.id})" style="
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (window as any).__hpSelectHospital = (id: number) => {
      onSelectHospital?.(id);
    };
    ...
    return () => {
      map.remove();
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      delete (window as any).__hpSelectHospital;
    };
  ...
  return (
    <>
      ...
      <div
        id="hp-map"
        style={{ width: "100%", height: "280px", borderRadius: "16px", overflow: "hidden", border: "1px solid #e2e8f0" }}
      />
    </>
  );
}
